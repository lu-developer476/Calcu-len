...
def test_scientific_accepts_special_characters_unicode():
    mode = client.post("/api/angle-mode", params={"mode": "DEG"})
    ...
    response = client.post(
        ...
        json={
            "mode": "scientific",
            "expression": "√(125^2 - 5^4) + log(1000) + sin(45) × 20",
        },
    )

    assert response.status_code == 200
    assert response.json()["result"] == "139.61662"

    restore = client.post("/api/angle-mode", params={"mode": "RAD"})
    assert restore.status_code == 200


def test_standard_accepts_decimal_comma():
    response = client.post(
        "/api/calculate", json={"mode": "standard", "expression": "4 + 2,5"}
    )

    assert response.status_code == 200
    assert response.json()["result"] == "6.50000"
